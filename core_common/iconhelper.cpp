...
{
    //主动绘制图形字体到图片
    QPixmap pix(width, height);//设置宽高
    pix.fill(Qt::transparent);//设置背景透明

    QPainter painter;//创建画家对象
    painter.begin(&pix);//画家在pix画布上作画
    painter.setRenderHints(QPainter::Antialiasing | QPainter::TextAntialiasing);//画家开启抗锯齿
    painter.setPen(color);//设置画家的画笔颜色

    iconFont.setPixelSize(size);//设置像素大小
    painter.setFont(iconFont);//设置字体为icon图形字体
    painter.drawText(pix.rect(), flags, (QChar)icon);//图形字体绘图
    //pix.load(":/image/LED.png");
    painter.end();
    return pix;
}
...
void IconHelper::setStyle1(QWidget *widget, QList<QAbstractButton *> btns, QList<int> icons, const IconHelper::StyleColor &styleColor)
{
    ...
    for (int i = 0; i < btnCount; i++) {
        int icon = icons.at(i);
        QPixmap pixNormal = getPixmap1(styleColor.normalTextColor, icon, iconSize, iconWidth, iconHeight);//未选中样式
        QPixmap pixHover = getPixmap1(styleColor.hoverTextColor, icon, iconSize, iconWidth, iconHeight);//鼠标悬浮样式
        QPixmap pixPressed = getPixmap1(styleColor.pressedTextColor, icon, iconSize, iconWidth, iconHeight);//鼠标按下样式
        QPixmap pixChecked = getPixmap1(styleColor.checkedTextColor, icon, iconSize, iconWidth, iconHeight);//选中样式
        ...
        btn->setIcon(QIcon(pixNormal));//把pixmap转换成icon然后设置
